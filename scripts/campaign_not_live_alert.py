...
SELECT
  ...
  SUM(
    CASE
      ...
      ELSE 0
    END
  ) AS HISTORICAL_IMPRESSIONS_BEFORE_LATEST_DAY,
  SUM(
    CASE
      WHEN d.DATE = l.LATEST_DELIVERY_DATE
      THEN d.IMPRESSIONS
  ...
  SUM(
    CASE
      ...
      ELSE 0
    END
  ) AS HISTORICAL_CLICKS_BEFORE_LATEST_DAY,
  SUM(
    CASE
      WHEN d.DATE < l.LATEST_DELIVERY_DATE
      THEN d.COST
      ELSE 0
    END
  ) AS HISTORICAL_COST_BEFORE_LATEST_DAY,
  SUM(
    CASE
      WHEN d.DATE < l.LATEST_DELIVERY_DATE
      THEN d.ROW_COUNT
      ELSE 0
    END
  ) AS HISTORICAL_ROWS_BEFORE_LATEST_DAY
FROM meta m
CROSS JOIN latest l
...
GROUP BY
  ...
  m.BOOKING_STATUS,
  l.LATEST_DELIVERY_DATE
HAVING HISTORICAL_IMPRESSIONS_BEFORE_LATEST_DAY > 200
   AND LATEST_DAY_IMPRESSIONS = 0
   AND PRIOR_DAY_IMPRESSIONS = 0
   AND LAST_NONZERO_IMPRESSIONS_DATE IS NOT NULL
ORDER BY LAST_NONZERO_IMPRESSIONS_DATE ASC, HISTORICAL_IMPRESSIONS_BEFORE_LATEST_DAY DESC, m.OUR_REF
"""
    ...
    for row in rows:
        ...
        alerts.append(
            AlertRow(
                ...
                total_impressions=float(row["HISTORICAL_IMPRESSIONS_BEFORE_LATEST_DAY"] or 0),
                total_clicks=float(row["HISTORICAL_CLICKS_BEFORE_LATEST_DAY"] or 0),
                total_cost=float(row["HISTORICAL_COST_BEFORE_LATEST_DAY"] or 0),
                row_count=int(row["HISTORICAL_ROWS_BEFORE_LATEST_DAY"] or 0),
            )
        )
    return alerts
...
def build_csv_stopped_impressions(rows: List[AlertRow]) -> bytes:
    ...
    writer.writerow(
        [
            ...
            "LAST_NONZERO_IMPRESSIONS_DATE",
            "HISTORICAL_IMPRESSIONS_BEFORE_LATEST_DAY",
            "HISTORICAL_CLICKS_BEFORE_LATEST_DAY",
            "HISTORICAL_COST_BEFORE_LATEST_DAY",
            "HISTORICAL_ROWS_BEFORE_LATEST_DAY",
        ]
    )
    for row in rows:
        writer.writerow(
